...
function publishNotice(){
	window.location="/home/notice/notice_edit.jsp";
}
...
function update(noticeId){
	window.location="/home/notice/notice_update.jsp?noticeId="+noticeId;
}

function deleteNotice(noticeId){alert(noticeId);
	$.post("/admin/deleteNoticeById.action",{"noticeId":noticeId},function(data){
		if(data.success){
			alert("删除成功！");
...
function searchNotice(pageNo){
	...
	$.ajax({
		type : "post",
		url : "/admin/searchMyPublishedNoticeList.action",
		data : {
		...
		success : function(data){
			...
			$.each(data.record,function(index,value){
				$("#data_list").append("<tr>"
						...
						+"<td><button type='button' class='btn btn-info btn-xs' data-toggle='modal' data-target='#findModal' onclick='detail("+value.noticeId+")'>查看</button>"
						+"<button type='button' class='btn btn-warning btn-xs' data-toggle='modal' data-target='#updateModal' id='btn_update' onclick='updateBook("+value.noticeId+")'>修改</button>"
