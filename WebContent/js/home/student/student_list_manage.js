...
function searchStudent(pageNo){
	...
	$.ajax({
		type : "post",
		url : "/student/searchStudentManageList.action",
		data : {
				"studentNoToSearch" : $("#studentNo").val(),
				"studentNameToSearch" : $("#studentName").val(),
				"sexToSearch" : $("#sex").val(),
				"birthplaceToSearch" : $("#birthplace").val(),
				"politicalLandscapeToSearch" : $("#politicalLandscape").val(),
				"majorNoToSearch" : $("#majorNo").val(),
				"cultivationTypeToSearch" : $("#cultivationType").val(),
				"page" : pageNo,
				},
		async : false,
		dataType: 'json',
		success : function(data){
			...
			var totalCountInfo = "<li>符合条件的学生记录共有"+data.totalCount+"条</li>";
			...
			/*$("#paginationFoot").append("<ul class='pagination'>" + headPage + pageFirst + upPage+pageMedia
					+ nextPage + pageLast + exportExcel + "</ul>");*/
			$("#paginationUl").append(totalCountInfo + headPage + pageFirst + upPage+pageMedia
