...
$(function(){
	blogId=$("#blogId").val();
	UEDITOR_CONFIG.imageUrl += "type=2&objectId=" + blogId;
    UEDITOR_CONFIG.catcherUrl += "type=2&objectId=" + blogId;   
	ueditor =  UE.getEditor('content',{autoHeightEnabled:false});	
	
	$.get("/student/getBlog.action",{"blogId":blogId},function(data){
		if(data.exist){
			$("#title").val(data.title);
			//$("#tagInput").val(data.);
			$("#referrenceBlogUrl").val(data.referrenceBlogUrl);
			ueditor.ready(function(){
				ueditor.setContent(data.blogContentText);    
			});
	    }else{
	    	alert("博客不存在！");
...
function addTag(){
	alert($("#tagInput").val());
	var str = "<div style = 'width:13%;float:left'>"
...
function submit(){
	   var content = ueditor.getContent();
		...
		//return;
		$.post("/admin/updateBlog.action",blog,function(data){
			if(data.msg){
				alert("修改成功！");
