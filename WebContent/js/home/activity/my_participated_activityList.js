...
function deleteBlog(activityId){
	alert(activityId);
	$.post("/admin/deleteBlog.action",{"activityId":activityId},function(data){
		if(data.msg){
			alert("删除成功！");
			window.location="/activity/myBlogList.action";
	    }else{
	    	alert("删除失败！");
...
function searchActivity(pageNo){
	...
	$.ajax({
		type : "post",
		url : "/student/serachMyParticipatedActivityList.action",
		data : {
		...
		success : function(data){
			...
			$.each(data.record,function(index,value){
				$("#data_list").append("<tr>"
						...
						+"<button type='button' class='btn btn-info btn-xs' data-toggle='modal' data-target='#findModal' onclick='participateIn("+value.activityId+")' >报名</button></td>"
						);
				});
