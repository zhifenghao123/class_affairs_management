    ...
    <head>
      ...
      <%@include file="/admin/included.jsp" %>
        ...
        <script type="text/javascript">
          var interioremployeeId = "${param.id}";
          $(function() {
            if (interioremployeeId.length > 0) {
              $.post("/admin/getInterioremployeeById.action", {
                id: interioremployeeId
              },
              function(result) {
                if (result.interioremployeeId != undefined) {
                  ...
                  function(data) {
                    ...
                    function(index, value) {
                      if (roleId == value.roleId) $("#ownRole").append("<li><input  type='radio' name='role' checked='checked' value='" + value.roleId + "' />" + value.name + "</li>");
                      else $("#ownRole").append("<li><input  type='radio' name='role' value='" + value.roleId + "' />" + value.name + "</li>");
                    });
                  },
                  "json");
                }
          ...
          var submitForm = function($dialog, $datagrid, $pjquery) {
            if ($('form').form('validate')) {
              ...
              var url;
              if (interioremployeeId.length > 0) {
                url = '/admin/updateinterIoremployee.action';
              } else {
                url = '/admin/addInterioremployee.action';
              }

              var data = {
                interioremployeeId: interioremployeeId,
                realname: $("#name").val(),
                jobno: $("#jobNo").val(),
          ...
          <form>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td colspan="5">
                  <input id="jobNo" name="input" type="text" class="putin1 black easyui-validatebox"
