    ...
    <head>
      ...
      <%@include file="/admin/included.jsp" %>
        ...
        <script type="text/javascript">
          ...
          $(function() {
            student_datagrid = $("#student").datagrid({
              url: "/admin/getGraduationProjectList.action",
              fit: true,
              pagination: true,
              fitColumns: true,
              nowrap: false,
              border: false,
              singleSelect: true,
              idField: 'graduationProjectId',
              columns: [[
              {
	             field: 'graduationProjectId',
	             title: '',
	             width: 40,
	             align:'center',
	             formatter : function(value,row,index) {
					var str="";
					//if(row.status=="2"){
						str="<input type='checkbox' name='studentCheckbox'" +"onclick='checke(\""+row.graduationProjectId+"\")'>";
					//}
					return str;
				}
	          },
              ...
              {
                 field: 'topicSelectingReportScore',
                 title: '开题报告得分',
                 width: 40
               },
               
			   {
                ...
                formatter: function(value, row, index) {
                  var stateButton;
                  if (row.stateText == "") {
                    stateButton = '<a href="javascript:void(0)" class="forbid" onclick="change_state(\'' + row.graduationProjectId + '\',2);"></a>';
                  } else {
                    stateButton = '<a href="javascript:void(0)" class="open" onclick="change_state(\'' + row.graduationProjectId + '\',1);"></a>';
                  }
                  var str = '<a href="javascript:void(0)" class="edit" onclick="edit_student(\'' + row.graduationProjectId + '\');"></a>';
                  str += '<a href="javascript:void(0)" class="delete" onclick="delete_student(\'' + row.graduationProjectId + '\');">|</a>';
                  str += stateButton;
                  return str;
                }
         ...
         function changeExcutiveClassBySelectedGrade(selectedGradeNo){
          ...
          var add_student = function() {
            addStudentDialog = parent.qujan.modalDialog({
              title: '新增专业',
              url: '/admin/informationManagement/graduationProjectForm.jsp',
              height: 450,
              width: 600,
              buttons: [{
                text: '返回',
          ...
          var edit_student = function(graduationProjectId) {
            editStudentDialog = parent.qujan.modalDialog({
              title: '修改专业',
              url: '/admin/informationManagement/graduationProjectForm.jsp?graduationProjectId=' + graduationProjectId,
              height: 450,
              width: 600,
              buttons: [{
                text: '返回',
       ...
       /*    var batch_add_student = function(){
        	  editStudentDialog = parent.qujan.modalDialog({
                  title: '批量导入学生',
                  url: '/admin/informationManagement/graduationProjectBatchImportForm.jsp',
                  height: 250,
                  width: 500,
                  buttons: [{
                    text: '返回',
        ...
        <div data-options="region:'north'" border="false" style="height:80px;overflow:hidden;">
          <form id="search_student_form">
            <div class="datagrid-toolbar search_1">
              <span>学号:</span>
              <input id="studentNo" type="text" size="7" onblur="search_student()"/>
              <span>姓名:</span>
              <input id="studentName" type="text" size="7" onblur="search_student()"/>
              <span>年级:</span>
